...
const emit = defineEmits<{
  (e: 'add-category' | 'delete-category' | 'add-subcategory', id: number): void
  ...
  (e: 'check-all-children' | 'check-all-section', id: number, month: number, isPaid: boolean): void
}>()

const { getSectionTotalForMonth } = useYearlyCategories()
const { getSectionPercentage, isSectionOverBudget } = useYearlySummary()
const { currentBudget: _currentBudget } = useYearlyBudget()
